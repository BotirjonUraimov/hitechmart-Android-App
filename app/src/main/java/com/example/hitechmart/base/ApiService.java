...
import com.example.hitechmart.BuildConfig;
        ...
        if (response.code() == 401) {  // check if response code is unathorized token
            ...
            try {
                ...
                if (loginResponse.code() == 200) {
                    ...
                    preferenceManager.setValue("access_token", user.getAccess_token());
                    requestBuilder.removeHeader("Authorization");
                    requestBuilder.addHeader("Authorization", "Bearer " + user.getAccess_token());
                    Request request = requestBuilder.build();
